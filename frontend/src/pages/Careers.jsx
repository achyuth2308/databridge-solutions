...
function Careers() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            setTimeout(() => { setShowModal(false); setSubmitStatus(null) }, 2000)
        } catch (error) {
            ...
            {showModal && (
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="fixed inset-0 z-50 flex items-center justify-center p-4">
                    ...
                    <motion.div initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }} className="relative w-full max-w-lg glass-card p-6 bg-white dark:bg-slate-900 max-h-[90vh] overflow-y-auto">
                        ...
                        {submitStatus && (
                            <div className={`mb-4 p-4 rounded-xl ${submitStatus === 'success' ? 'bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400' : 'bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-400'}`}>
                                {submitStatus === 'success' ? 'Application submitted successfully!' : 'Error submitting. Please try again.'}
                            </div>
                        )}
